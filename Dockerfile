...
# This is the base image so we want to ensure we have the latest security updates.
RUN apt-get update && \
...
# Set up the Chrome repository - Chrome is about 130 mb smaller than chromium
...
RUN rm -rf /usr/local/lib/ruby/gems/$(ruby -e 'print RUBY_VERSION[/\d+\.\d+\.\d+/]')/specifications/default
